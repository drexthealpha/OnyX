...
describe("Auth middleware", () => {
  ...
  afterAll(() => {
    if (originalToken === undefined) delete process.env.ONYX_NERVE_TOKEN;
    else process.env.ONYX_NERVE_TOKEN = originalToken;
  });
...
describe("broadcastEvent", () => {
  test("formats and sends correct SSE data string to all connected clients", async () => {
    ...
    const fakeWriter = { write: (chunk: string) => { written.push(chunk); } };

    writers.add(fakeWriter as unknown as WritableStreamDefaultWriter);
    broadcastEvent("intel", { topic: "test" });
    writers.delete(fakeWriter as unknown as WritableStreamDefaultWriter);
