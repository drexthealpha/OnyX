...
Part of the ONYX RL Python runtime.
Used by generate_with_swe.py (inside the RolloutManager) and standalone
...
class OnyxSweEnvClient:
    ...
    async def _post(self, path: str, payload: dict[str, Any], max_retries: int,
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if not isinstance(output, dict):
                    raise RuntimeError(f"ONYX SWE request returned non-dict payload: {type(output).__name__}")
                return output
            except Exception:
                retry_count += 1
        ...
        raise RuntimeError(f"ONYX SWE request failed after retries: {url}")
    ...
    async def exec(
        ...
        if not out.get("ok", False):
            raise RuntimeError(f"ONYX SWE exec failed: {out}")
        return out
    ...
    async def diff(self, lease_id: str, cwd: str = "/testbed") -> str:
        ...
        if not out.get("ok", False):
            raise RuntimeError(f"ONYX SWE diff failed: {out}")
    ...
    async def evaluate(
        ...
        if not out.get("ok", False):
            raise RuntimeError(f"ONYX SWE evaluate failed: {out}")
        return out

    async def close(self, lease_id: str) -> None:
        out = await self._post("/close", {"lease_id": lease_id}, self.default_max_retries)
        if not out.get("ok", False):
            raise RuntimeError(f"ONYX SWE close failed: {out}")
        return None


# Backwards-compatible alias
SweEnvClient = OnyxSweEnvClient