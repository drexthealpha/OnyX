...
export function initTradingWS(server) {
    const wss = new WebSocketServer({ server: server, path: "/ws/trading" });
        ...
        const interval = setInterval(async () => {
            try {
                ...
                try {
                    ...
                    const res = await fetch(`https://public-api.birdeye.so/defi/price?address=${token}`, { headers: { "X-API-KEY": key } });
