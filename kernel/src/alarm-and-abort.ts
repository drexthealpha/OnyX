...
import { type AbortHandler, AlarmCode } from "./types.js";
...
/**
 * alarm — non-abortive notification of a critical condition.
 * Records the event in the phase log and continues execution.
 * (AGC analogue: recording the code in FAILREG and returning).
 */
export function alarm(service: string, code: AlarmCode, metadata?: Record<string, unknown>): void {
  phaseLog({
    service,
    from:      "active",
    to:        "active",
    timestamp: Date.now(),
    metadata:  { alarmCode: code, ...metadata },
  });
}
