"""ONYX SWE PRM (Process Reward Model) for step-wise evaluation.
...
class SweRewardAgent:
    ...
    async def _query_prm_once(
        ...
        logger.info(
            f"{_ANSI_RED}[ONYX-SWE-PRM]{_ANSI_RESET} step=%s vote=%s in_len=%s output:\n%r",
    ...
    async def collect_step_results(
        ...
        for (step_idx, _), result in zip(pending_tasks, done, strict=False):
            if isinstance(result, Exception):
                logger.warning(f"[ONYX-SWE-PRM] step {step_idx} exception: {result}")
                step_details.append(
