...
const VISION_PROMPT =
  "Describe this image in markdown. Include all text visible in the image. " +
...
export async function parse(filePath: string): Promise<string> {
  ...
  if (!apiKey) {
    throw new Error(
      "ANTHROPIC_API_KEY is required for image parsing. Set it in your environment."
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Claude vision API error ${response.status} for "${path.basename(filePath)}": ${errorText}`
    );
  }
