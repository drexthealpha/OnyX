"""ONYX Terminal-RL Agent Runner.

Ported from Gen-Verse/OpenClaw-RL terminal-rl/agent_runner.py.
...
logger = logging.getLogger("onyx.terminal_rl.agent_runner")
...
class AgentRunner:
    ...
    async def run_episode(self, user_msg: Any) -> RolloutOutcome:
        ...
        try:
            ...
            logger.info(
                "%s [ONYX] Rollout finished: turns=%d parse_errors=%d iteration_limit=%s parse_error_limit=%s",
                self._log_tag,
                self._model_turn_count,
