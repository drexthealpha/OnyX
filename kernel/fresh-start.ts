...
import { RESTART_DEBOUNCE_MS, MAX_RESTART_ATTEMPTS } from "./constants.ts";
import { AlarmCode } from "./types.ts";
import { phaseLog } from "./phase-table.ts";
import { abort, register } from "./alarm-and-abort.ts";
...
export async function watchdog(
  ...
  while (attempts < MAX_RESTART_ATTEMPTS) {
    ...
    try {
      ...
      const { clear } = await import("./alarm-and-abort.ts");
      clear(abortId);
      return;
