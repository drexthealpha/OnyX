...
Part of the ONYX RL Python runtime (ported from Gen-Verse/OpenClaw-RL).
...
from swe_env_client import OnyxSweEnvClient as SweEnvClient
...
def _save_rollout_artifacts(*, sample: Sample, iid: str, sampling_params: dict, run_info: dict) -> None:
    ...
    try:
        ...
        traj_payload = {
            ...
            "trajectory_format": "onyx-swe-trajectory-1",
        }
        ...
        logger.info(f"[ONYX-SWE] [{iid}] Saved rollout artifacts to {run_dir}")
    except Exception as e:
        logger.warning(f"[ONYX-SWE] [{iid}] Failed to save rollout artifacts: {e}")
...
def _resolve_eval_script(instance: dict) -> str:
    ...
    if kind == "swebench_multimodal":
        try:
            ...
            logger.error("[ONYX-SWE] [{}] Cannot import swebench make_test_spec for multimodal eval", iid or "unknown")
            return ""
    else:
        try:
            ...
            try:
                ...
                logger.error("[ONYX-SWE] [{}] Cannot import swegym/swebench make_test_spec; eval_script unavailable", iid or "unknown")
                return ""
    ...
    try:
        ...
        if isinstance(script, str) and script.strip():
            ...
            return script
        logger.error("[ONYX-SWE] [{}] make_test_spec returned empty eval_script", iid or "unknown")
        return ""
    except Exception as e:
        logger.exception("[ONYX-SWE] [{}] Failed to build eval_script: {}", iid or "unknown", e)
        return ""
...
def _ensure_openai_base_url(args) -> None:
    ...
    if router_ip and router_port:
        ...
        logger.info(f"[ONYX-SWE] OPENAI_BASE_URL resolved to {url}")
    ...
    *,
    ...
    for step_idx in range(step_limit):
        ...
        if max_context_len > 0 and len(full_input_ids) + max_new_tokens > max_context_len:
            ...
            logger.warning(f"[ONYX-SWE] [{iid}] {error}")
            break
        ...
        try:
            ...
            logger.error(f"[ONYX-SWE] [{iid}] {error}")
            exit_status = "error"
            break
        ...
        try:
            ...
            logger.warning(f"[ONYX-SWE] [{iid}] step {step_idx} timed out after {step_timeout}s")
        except Exception as e:
            returncode = -1
            output_text = f"Execution error: {e}"
            logger.error(f"[ONYX-SWE] [{iid}] step {step_idx} exec error: {e}")
    ...
    logger.info(
        f"[ONYX-SWE] [{iid}] Agent done: steps={n_steps}, exit={exit_status}, "
...
async def _generate_impl_trajectory(args, sample: Sample, sampling_params: dict) -> Sample:
    ...
    logger.info(
        "[ONYX-SWE] ========== TRAJECTORY ROLLOUT ENTERED ========== instance_id={} | "
    ...
    logger.info(f"[ONYX-SWE] [{iid}] Step 1/5: Waiting for semaphore...")
    await swe_semaphore.acquire()
    logger.info(f"[ONYX-SWE] [{iid}] Step 1/5: Semaphore acquired ({time.time()-t_start:.1f}s)")
    ...
    try:
        logger.info(f"[ONYX-SWE] [{iid}] Step 2/5: Allocating container for {image_name}")
        lease = await env_client.allocate(image=image_name, instance_id=iid)
        lease_id = lease["lease_id"]
        logger.info(f"[ONYX-SWE] [{iid}] Step 2/5: Container ready, lease={lease_id}")
        ...
        logger.info(f"[ONYX-SWE] [{iid}] Step 3/5: Running agent (trajectory mode)...")
        ...
        if git_patch and skip_eval:
            logger.info(f"[ONYX-SWE] [{iid}] Step 4/5: SWE_SKIP_EVAL=1, skipping eval")
        elif git_patch:
            ...
            if policy.get("violated", False):
                ...
                logger.warning(f"[ONYX-SWE] [{iid}] Step 4/5: Policy blocked patch: {reason}")
                run_info["reward"] = 0
                ...
                logger.error(f"[ONYX-SWE] [{iid}] Step 4/5: eval_script unavailable, skipping eval")
            else:
                try:
                    await env_client.close(lease_id)
                    logger.info(f"[ONYX-SWE] [{iid}] Step 3/5: Closed agent container lease={lease_id}")
                    lease_id = None
                except Exception:
                    logger.exception(f"[ONYX-SWE] [{iid}] Failed to close agent lease before eval")

                logger.info(f"[ONYX-SWE] [{iid}] Step 4/5: Allocating fresh eval container...")
                eval_result = None
                try:
                    ...
                    logger.error(f"[ONYX-SWE] [{iid}] Step 4/5: Eval error: {e}")
                finally:
                ...
                if eval_result is not None:
                    ...
                    try:
                        ...
                        logger.warning(f"[ONYX-SWE] [{iid}] Harness grading failed, fallback to returncode: {ge}")
                    ...
                    logger.info(
                        f"[ONYX-SWE] [{iid}] Step 4/5: resolved={resolved} "
                        f"(returncode_resolved={resolved_by_returncode})"
                    )
        else:
            logger.warning(f"[ONYX-SWE] [{iid}] Step 4/5: No patch, skipping eval")

    except Exception as e:
        run_info["error"] = str(e)
        logger.exception(f"[ONYX-SWE] [{iid}] Error: {e}")
    finally:
        if lease_id is not None:
            try:
                await env_client.close(lease_id)
            except BaseException:
                logger.warning(f"[ONYX-SWE] [{iid}] Failed to close lease (may be cancelled)")
        swe_semaphore.release()
        logger.info(f"[ONYX-SWE] [{iid}] Semaphore released")
    ...
    if not messages:
        logger.warning(f"[ONYX-SWE] [{iid}] Step 5/5: ABORTED — no messages (error={error})")
        sample = copy.deepcopy(sample)
        sample.status = Sample.Status.ABORTED
        sample.reward = {"score": 0.0, "acc": 0.0}
        sample.remove_sample = True
...
async def generate_trajectory(args, sample: Sample, sampling_params: dict) -> Sample:
    ...
    try:
        ...
        logger.error(
            f"[ONYX-SWE] [{iid}] TOTAL ROLLOUT TIMEOUT ({rollout_timeout}s) exceeded, aborting sample"
        )
        sample = copy.deepcopy(sample)
        sample.status = Sample.Status.ABORTED
        sample.reward = {"score": 0.0, "acc": 0.0}
