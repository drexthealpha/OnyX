...
export async function loadPlugin(modulePath: string): Promise<OnyxPlugin> {
  ...
  if (!plugin.name || !plugin.init || !plugin.execute) throw new Error(`Invalid plugin at ${modulePath}: missing name, init, or execute`);
...
export async function loadPluginsFromDir(dir: string): Promise<OnyxPlugin[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try { loaded.push(await loadPlugin(`${dir}/${entry}`)); } catch {}
    }
  } catch {}
  return loaded;
}