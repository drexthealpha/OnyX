"""ONYX Tool Sandbox — safe code execution and tool management.

Ported from Gen-Verse/OpenClaw-RL toolcall-rl/tool_sandbox.py.
All original logic preserved. ONYX branding applied.
...
TOOL_CONFIGS = {
    ...
    # ONYX: max observation characters per tool result
    "max_obs_chars": 1024,
}
...
class PythonSandbox:
    """ONYX Python code sandbox — safe code execution environment."""
    ...
    def _create_safe_environment(self):
        ...
        temp_dir = tempfile.mkdtemp(prefix="onyx_sandbox_")
    ...
    async def execute_code(self, code: str) -> str:
        """Execute Python code in ONYX sandbox with safety checks"""
        current_memory = get_memory_usage()
...
class ToolRegistry:
    """ONYX Tool Registry — manages available tools and their execution."""
    ...
    def _register_default_tools(self):
        """Register default ONYX tools in the registry"""
        self.register_tool(
            "code_interpreter",
            {
                "type": "function",
                "function": {
                    ...
                    "description": "ONYX tool for executing Python code in a safe sandbox environment.",
                    "parameters": {
                        "type": "object",
                        "properties": {
    ...
    def register_tool(self, name: str, tool_spec: dict[str, Any]):
        """Register a new tool in the ONYX registry"""
    ...
    async def execute_tool(self, tool_name: str, arguments: dict[str, Any]) -> str:
        ...
        if tool_name not in self.tools:
            return f"Error: Tool '{tool_name}' not found in ONYX registry"
    ...
    async def _execute_python(self, arguments: dict[str, Any]) -> str:
        """Execute Python code using the ONYX sandbox"""
...
# Global ONYX tool registry instance
